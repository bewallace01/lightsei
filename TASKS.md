...
> **Phase 30.4 — dashboard capability port (first surface).** 30.3 shipped 2026-05-30 across six sub-commits (`c142237` schema, `7d542ec` Polaris router, `f640b3e` endpoints+dispatch, `10209ac` claim/chunk/complete, `b06e71d` SDK dual-surface poller, `43b8b26` iOS team channel UI). Polaris-routed team channel is end-to-end provable: 46 new tests pass; the SwiftUI shell renders user / "Polaris" / per-bot attribution rows; compile-green on iPhone 17 sim. Visual round-trip verify deferred until a deployed bot actually claims a team turn (operator-driven). 30.4+ ports operator dashboard surfaces into the iOS app one at a time; spec candidate order is agents list → agent detail → runs → cost → zones → integrations → settings, lowest-friction first. **Decision pending: which surface starts 30.4?** Parking-lot bug #290 still open (PWA /c install banner on iOS Safari prod).
>
