...
> **Phase 30.8 — integrations surface (fifth iOS dashboard port).** 30.7 (Zones) shipped 2026-05-31 across three sub-commits (`4076a0c` Zones view, `334c5a7` 5-segment picker, `9aea8cb` picker shape refactor — replaced the segmented control with a horizontally-scrolling pill strip so 30.8 + 30.9 can land without cramping). Adding a new surface is now an enum case + one row in `paneModeOrder`. 30.8 ports the integrations index: list of installed connectors (Slack workspace + Google Gmail/Calendar/Drive — Phase 19 + 20 surfaces) with per-connector status (connected / expired / never installed). Audit pending: backend has `/connectors/installations` + per-connector start/disconnect endpoints (`backend/connectors/`); web /integrations is a thin list. Read-only first iteration (no installing from mobile — OAuth flow on phone is fragile; the web's the right surface to start a connection).
>
