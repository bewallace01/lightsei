...
> **Phase 30.5 — agents list + detail (second iOS dashboard surface).** 30.4 (Runs) shipped 2026-05-30 across five sub-commits (`e57373c` backend `with_summary` aggregator + 7 tests, `c109562` iOS API client + URL-query patch + `JSONValue`, `d63fa91` Runs list view, `e11dddc` run detail view, `d587ffa` Channels|Runs segmented control). Operator shell now flips between Channels and Runs per-workspace; the Runs list shows status dot + agent + relative time + latency/tokens, taps push into a detail view with expandable JSON event cards. Backend `?with_summary=true` is the N+1-killer the mobile depends on. Visual round-trip needs operator login (deferred). 30.5 ports the next surface from the spec's candidate order: agents list + agent detail (read-only first iteration; editing comes later). Picker grows to `Channels | Runs | Agents`. Sub-task breakdown TBD after a quick audit of the existing GET /agents response shape (most fields likely already there; backend may need no change).
>
