...
> **Phase 12D.2: Polaris emits a periodic cost-analysis event**

Phase 12C closed 2026-05-17 — the team-from-README demo ran end to end on prod (worker deployed to Railway, polaris ticking, team bots claimed + running, push-to-dispatch chain visible on the constellation). Both 12C.5 (the original demo) and 12C.6 (the async-generation hardening that unblocked it) fold into the same shipped feature; the Done Log captures both arcs.

12D is the next phase: Lightsei stops being a passive cost recorder and becomes an active cost optimizer. 12D.1 (read-only spending audit at `/cost/insights`) shipped 2026-05-05; 12D.3 (auto-tuning with consent) is gated on Phase 14 (continuous eval) for a quality signal. NOW is 12D.2: fold the same heuristics 12D.1 surfaces on a dedicated page into a periodic `polaris.cost_analysis` event the home page renders next to `polaris.plan`. The point is to put the audit *in front of* the user during their normal review flow rather than waiting for them to navigate to a separate insights page they'll forget exists.

Practical concern motivating the priority: 2026-05-17 prod snapshot showed `vela` ticking every ~9 min (78 runs/24h), generated by the team-from-README pipeline with an aggressive `@lightsei.on_schedule` default. 12D.2 would surface that as "vela is producing identical output across runs; suggest 1h interval" inline. Until then it sits silently in `/cost/insights` (if it's flagged at all) or compounds spend invisibly.
