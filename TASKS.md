...
> **⬅ ACTIVE THREAD = Phase 37 (SEO assistant + design-match + git-deploy publishing).** This is the workstream of the last several sessions (founder-driven, 2026-06-17 → 06-23). The older iOS marker (31.6.d "await Apple review") is STALE — it dates to 2026-06-07 and needs a status check (likely approved or needs a resubmit; nobody has reconciled it). Don't treat 31.6.d as the live NOW.
>
...
>   - **37.10b DB wiring** ⚠️ code-complete, integration tests UNRUN (need Docker). Migration 0057 (`workspaces.seo_autopublish_enabled` default false + `seo_autopublish_repo_id` nullable; new single head 0057), Workspace model cols, `post_event` ingest hook (schedules graceful background `_autopublish_seo_draft` on `seo.page_drafted` only when opted-in + repo set), and `GET/PATCH /workspaces/me/seo/autopublish` (validates the repo via `_resolve_publish_target`; rejects enabling without a target). Tests written in `tests/test_seo_autopublish_endpoint.py` (settings GET/PATCH + ingest-hook fires/doesn't); all backend files byte-compile. **TO VERIFY:** run the backend suite once Docker is available — at minimum `pytest tests/test_seo_autopublish_endpoint.py tests/test_github_publish.py tests/test_seo_endpoint.py` (confirms the ingest hook + the endpoint refactor didn't regress) + a single-alembic-head check.
...
>
> **37.NEXT (after 37.10) — other SEO-thread forks not yet chosen:** (b) **non-React frameworks** for preview/auto-route (shell-preview + route-wiring assume Vite + React Router with a central App.tsx); (c) **CMS connector** (WordPress/Webflow REST) for sites NOT hosted from a git repo.
>
