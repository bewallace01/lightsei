...
> - 31.5 ⬅ NOW — App Store metadata + privacy + reviewer creds. Drafting done 2026-06-05 (`ios/AppStore/`, PR #34); what remains is manual App Store Connect web-UI entry + screenshots (not Claude-doable).
...
> - 31.6 ⬅ NEXT — Submit + iterate on App Store review:
>   - 31.6.a — Merge PR #34, then cut an `ios-v*` tag so a build carrying the export-compliance key uploads to App Store Connect; select that build for the version.
>   - 31.6.b — Verify on that build: demo operator account signs in via Business > "Use password instead", and the workspace shows >= 1 bot + a real conversation. Confirm the listing copy's named surfaces (chat, Activity/Runs view) actually render so metadata matches the app.
