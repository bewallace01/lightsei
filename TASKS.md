...
> **⬅ ACTIVE THREAD = Phase 37/38/39 (SEO assistant + design-match + git-deploy publishing + CMS).** Founder-driven, 2026-06-17 → 06-25. As of 2026-06-25 the stack is merging to main (PRs #141, #144 in; #140 here; #143 WordPress next). The older iOS marker (31.6.d "await Apple review") is STALE — it dates to 2026-06-07 and needs a status check; don't treat it as the live NOW.
>
> **Phase 38: framework support for publishing.**
>   - **38.2a Framework detection + format defaulting** ✅ done (PR #144, 2026-06-25). The publish UI defaults the page format to the repo's framework. Pure `backend/framework_detect.py`: `detect_framework` → `next-app`/`next-pages`/`vite-react`/`static`/`unknown` (+ `default_format_for`); endpoint `GET /workspaces/me/github/repos/{id}/framework`; dashboard `fetchRepoFramework` + a DraftCard effect that defaults the format on form-open (respecting a manual pick via `formatEdited`). 9 pure tests + endpoint tests; CI green.
>   - **38.1 Next.js render targets** ✅ done (PR #141, 2026-06-24). `seo_render`: `next-app` (→ `app/<slug>/page.tsx`, server component + `export const metadata`) + `next-pages` (→ `pages/<slug>.tsx`, `next/head`). Body HTML via `dangerouslySetInnerHTML` with JSON-escaped literals. Next.js needs NO App.tsx route-wiring (file-based routing). 4 tests; CI green.
>   - **38.2b Auto-publish renders to the repo's framework** ✅ done (2026-06-25). `_autopublish_seo_draft` no longer hardcodes `fmt="html"`: it detects the repo's framework server-side (the background path has no UI, unlike the manual path which detects in the dashboard) via `github_publish.fetch_file(package.json)` + `list_page_files` → `framework_detect.detect_framework` → `default_format_for`, falling back to html on any error. 2 DB-backed tests (Next.js repo → `next-app`; detection error → `html`). Migration-free.
>
> **37.11 Fix: publish branch-collision is a dead-end error** ✅ done (PR #141, 2026-06-24, found in prod). On the branch-create 422, `github_publish.publish_page_to_repo` looks up the existing open PR (`find_open_pr_for_branch`) and returns `{...,already_open:true}` instead of erroring — the `/seo` card links to the open PR. No-open-PR collision → a clearer error. `_httpx_request` preserves JSON-array bodies. 2 pure tests; CI green.
>
> **Phase 39: CMS publishing (WordPress) — IN PROGRESS (PR #143).** 39.1 WordPress publish core (`backend/wordpress_publish.py`, pure, REST + Application Password, draft-by-default; 13 pure tests). NEXT: 39.2 credential storage + endpoint, 39.3 dashboard. See [[seo-assistant-and-git-deploy-publish]], [[alembic-migration-numbering-parallel-branches]].
>
> **Phase 37: SEO assistant (Spica), design-match (Capella), and git-deploy publishing.** Spica drafts SEO pages, Capella matches the site's look, and "publish" = commit the page to the owner's connected GitHub repo + open a PR (one mechanism for every git-hosted host: Vercel/Cloudflare/Railway/Netlify). The last 6 PRs (#134-139) hardened the in-shell preview so an owner sees a faithful render of a repo-matched (component) page before publishing.
> - **37.9 Preview: auto-borrow the matched template page's shell** ✅ done (PR #139, 2026-06-22). The shell preview used the site-URL field (defaults to homepage), whose layout differs from an article page, so previews looked wrong unless the owner pasted a content-page URL. Now in component mode it derives the shell from the SELECTED template's live URL: fetch the template file → component name → its registered route in `App.tsx` (`react_router_wiring.route_for_component`, honors custom SEO slugs) → join to the site origin. The URL field now only supplies the origin; falls back to it if resolution fails. Dashboard passes `template_repo_id`/`template_path` to `/workspaces/me/design/preview-in-shell`. 9 `test_shell_preview.py` tests green (verified locally 2026-06-23).
> - **37.8 Preview: mirror the shell's content wrapper + heading classes** ✅ done (PR #138). `shell_preview.mirror_structure` reads the wrapper element + h1/lead/section classes from the shell's original `<main>` and rebuilds the new content with them (column width, serif headings, per-H2 section wrappers), falling back to a plain fragment when there's no recognizable wrapper.
> - **37.7 Preview: load the site's CSS** ✅ done (PR #137). Strip `crossorigin` from the shell (so the cross-origin stylesheet isn't CORS-gated and blocked) + rewrite root-relative `href`/`src` to absolute against the site origin; `<base>` stays as a fallback. Preview now loads real fonts/colors/layout. (Plus #135: send the JSON content-type header on the preview POST.)
> - **37.6 SEO drafts: delete + preview-in-shell spine** ✅ done (PRs #134, #136). `POST /workspaces/me/design/preview-in-shell` renders a repo-matched page inside the live site shell (`backend/shell_preview.py`: strip scripts, inject `<base>`, swap `<main>`); owners can delete a draft from `/seo`.
> - **37.5 Publish: auto-register the page route in App.tsx (React Router repos)** ✅ done (PR #132). `backend/react_router_wiring.py` (pure): for Vite + React Router repos with a central `src/App.tsx`, publishing a `src/pages/*.tsx` page also wires the `lazy` import + `<Route>` (PascalCase→kebab, inserted before the `path="*"` catch-all, idempotent) in the SAME PR, so the page is reachable on merge. `publish_page_to_repo` gained `extra_files` (companion files committed before the PR).
> - **37.4 Design match: by-role colors + theme detect** ✅ done (PR #131). `backend/design.py extract_style_profile()` reads colors BY ROLE (body/html bg, body text, h1-h3, a) + detects light/dark theme (background luminance, Tailwind `bg-*`/`text-*` tally fallback), stops frequency-counting every hex (which made Tailwind light sites build dark pages). Folds same-origin external stylesheets in before extraction.
> - **37.3 Design component mode + template picker** ✅ done (commits 6259c9f #127, 6504338 #129). `content_type='component'` matches a real page file fetched from the connected repo (`github_publish.fetch_file`) — the most faithful path (mirrors framework/imports/layout/nav/theme). `GET /workspaces/me/github/repos/{id}/page-files` lists candidate page files; dashboard `/seo` template-picker UI.
> - **37.2 GitHub connect: pasted-token → OAuth upgrade** ✅ done (PR #130). A workspace with a read-only PAT GithubConnection can "Switch to OAuth" to get `repo` write scope (fixes 403 on publish). See [[github-app-not-oauth-app]].
> - **37.1 Spica SEO persona + git-deploy publish** ✅ done (PRs #104/#106/#107, 2026-06-17). 7th business persona `agents/seo/bot.py` (id `seo`, LLM-backed): `seo.audit` (heuristic on-page audit → 0-100 + findings, driven by the `seo_audit` url_target feeder, default ON, no key) + `seo.generate_page` (LLM drafts title/meta/slug/h1/body_html, needs ANTHROPIC_API_KEY). Events `seo.audit_complete`/`seo.page_drafted`/`seo.crash` wired into feed + digest. Publishing: `backend/github_publish.py publish_page_to_repo` + `POST /workspaces/me/github/publish-page` (commit page to connected repo + open PR; owner-initiated from `/seo`). CMS platforms (WordPress/Webflow/Shopify) are a separate REST-connector model, not yet built.
>
> **⬅ NOW: 37.10 Spica auto-opens the publish PR** (chosen 2026-06-23) — CODE-COMPLETE; one verification gap (DB-backed tests unrun, see 37.10b). Make publishing proactive: when a workspace opts in and picks a target repo, a new SEO page draft (`seo.page_drafted`) auto-opens a publish PR, reusing the existing git-deploy path. Default OFF; the opt-in is the standing authorization and the PR/merge stays the owner's review gate (nothing goes live without a merge). The backend publishes on event ingest (token lives backend-side; the bot stays credential-free); dedup is free via the deterministic branch name (a re-emitted draft collides 422 and no-ops).
>   - **37.10a Pure publish-orchestration core + endpoint refactor** ✅ verified (2026-06-23). `backend/seo_autopublish.py` (`should_autopublish`, `page_from_event`, `title_for`, `orchestrate_publish` with an injected GitHub `request` seam); `POST /workspaces/me/github/publish-page` refactored to call it (behavior-preserving — the existing endpoint tests monkeypatch `github_publish.publish_page_to_repo`, which the refactor still calls through). 10 pure tests in `tests/test_seo_autopublish.py`, all green (run via the scratch harness since Docker is down — `backend/tests/conftest.py` spawns Postgres in Docker on import).
>   - **37.10b DB wiring** ✅ verified in CI (2026-06-25, PR #140 backend green). Migration 0057 (`workspaces.seo_autopublish_enabled` default false + `seo_autopublish_repo_id` nullable; single head 0057), Workspace model cols, `post_event` ingest hook (schedules graceful background `_autopublish_seo_draft` on `seo.page_drafted` only when opted-in + repo set), and `GET/PATCH /workspaces/me/seo/autopublish` (validates the repo via `_resolve_publish_target`; rejects enabling without a target). `tests/test_seo_autopublish_endpoint.py` (settings GET/PATCH + ingest-hook fires/doesn't) passed in the CI Postgres suite — the local Docker-down gap is closed by CI.
>   - **37.10c Dashboard** ✅ verified (2026-06-23). Auto-publish toggle + target-repo picker on `/seo` (`AutopublishPanel`); `fetchSeoAutopublish`/`setSeoAutopublish` in api.ts. tsc clean + `next build` green.
>
>
> **Fix: persona result events were silently dropped (broke feed + ask box + digest)** ✅ done (PR #94). Found during prod verification: the personas call `lightsei.emit("bi.summary"/"reputation.analyzed"/...)` OUTSIDE any run (the LLM-call run has already ended), and `_client.emit` DROPS events with no run context. So Altair answered the ask box but the `bi.summary` event never persisted → no answer shown; same for every persona's feed events. Fix: each persona mints a `run_id` per command and passes `run_id=` to its emits (the backend creates the Run row from that id on ingest — verified in `/events`). All 6 bots + re-vendored; 91 persona/drift tests green. (Root-cause note: a cleaner future fix is making `emit()` mint a run_id instead of dropping, but that needs an SDK PyPI release; the bot-level fix ships via the bundle we control.)
>
> **Phase 36: Embeddable website chatbot — branding/customization (founder ask 2026-06-13).** The "AI Website Assistant" (Product #1) embed already exists (Phase 21: `widget.js` loader + `/widget/{publicId}` chat + "powered by" footer). Founder wants owners to name + style the widget for their own site, with a "Powered by Lightspace Labs" badge that paid plans can remove.
> - **36.3 Launcher-bubble accent color** ✅ done (PR #90). The floating chat bubble on the customer's page now matches the owner's accent. Done via the existing iframe→loader postMessage bridge (NO cross-origin fetch / CORS): the widget page posts `{type:"lightsei:widget-theme", accent}` once config loads; `widget.js` validates it's a hex color and applies it to the launcher background + tinted shadow. Only posted when a custom color is set (else the loader keeps default indigo). node -c + tsc + next build green.
> - **36.2 "Powered by Lightspace Labs" badge (removable on paid)** ✅ done (PR #89). Footer rebranded Lightsei → "Powered by Lightspace Labs". `workspaces.widget_hide_branding` (migration 0056, default false). Public config returns `branding.show_powered_by = not (hide_branding and plan_tier=='paid')` — free always shows; paid can hide; downgrade auto-re-shows. PATCH accepts `widget_hide_branding` (stored freely, gated at render); GET returns it + `can_remove_branding` (paid-only). Settings: a "Remove the badge" checkbox, locked on free with an "(upgrade to remove)" link. 4 tests (free-always-shows, paid-hides-when-opted-out, paid-shows-by-default, toggle+gate). migration head 0056; tsc + next build green.
> - **36.1 Widget customization (name/color/greeting)** ✅ done (PR #88). `workspaces.widget_title`/`widget_accent_color`/`widget_greeting` (migration 0055, nullable=default). `PATCH /workspaces/me/widget-settings` extended (hex-color validated, empty clears); GET + public `/widget/{id}/config` return a `branding` object (and the public bot name is now the constellation display name, not the raw id). Widget page: custom title in header, accent on avatar+send button, greeting as opener. Settings page: a "Branding" section (name / color picker / greeting). 9 tests. migration head 0055; tsc + next build green. NEXT 36.2: "Powered by Lightspace Labs" + paid-gated remove-branding toggle. Follow-up: launcher-bubble color in widget.js (needs cross-origin config fetch / CORS).
>
> **Phase 35: Constellation assistant names (differentiation) — founder direction 2026-06-12.** Generic functional names (Inbox/Reputation/BI) aren't a differentiator; lean into the brand (Lightsei = a constellation of assistants). Each assistant gets a distinctive star name ALWAYS paired with its plain-English role ("Lyra · Reputation") so "who does what" stays obvious, and owners can rename them. Display-only: internal ids (`bi`,`inbox`,…) never change.
> - **35.4 Carry names to the constellation map (+ no-Polaris center)** ✅ done (PR #95). Found during prod use: the home/`/agents` constellation map still rendered raw ids (`website`,`bi`…) — the one surface missed in 35.x. `/constellation` endpoint now returns `display_name` + `assistant_role` (resolved via assistant_identity); the map label + tooltip use them. Also: business workspaces have no Polaris (retired dev orchestrator), so the map's center "sun" was an empty glow — now anchored with a faint "your team" core when no orchestrator is present. 1 test; tsc + next build green.
> - **35.3 Carry names to /agents + ask box** ✅ done (PR #87). Constellation identity now everywhere assistants surface, not just team-panel + feed. `_serialize_agent` returns `display_name`/`assistant_role`/`is_custom_name`; the /agents roster renders "Lyra · Reputation" (linked by id). Ask answers are attributed to the answering assistant ("Altair") — `ask.answering_assistant` resolves BI's identity (+ rename) and rides the POST/GET/list ask responses; AskBox shows the name above the live answer + history. 5 tests (agents identity + rename, ask attribution + rename follows). tsc + next build green.
> - **35.2 Per-workspace assistant rename** ✅ done (PR #84). Owners can rename any assistant; blank resets to the star default. `agents.display_name` (migration 0054, nullable, display-only — PK `name` untouched). `PATCH /workspaces/me/assistants/{name}` sets it (404 if not provisioned). team-status + feed read the override (feed endpoint passes a per-workspace name map into `build_feed_item`); both carry `is_custom_name`. Inline "rename" on the welcome team panel (hover → edit → save, Enter/Escape). 10 tests (rename→status, blank-reset, 404, feed-reflects-rename). migration head 0054; tsc + next build green.
> - **35.1 Constellation defaults + role labels** ✅ done (PR #83). `backend/assistant_identity.py`: `DEFAULT_IDENTITY` (website→Rigel, lead→Orion, reputation→Lyra, marketing→Nova, bi→Altair, inbox→Mira) + `identity(agent, override)` + `display_label` ("Name · Role"). Wired into the feed (`build_feed_item` now returns assistant_name/role/label, override-ready) and team-status endpoint (display_name + role per assistant). Welcome team panel shows "Name · Role"; cosmetic polish (lowercased goal labels mid-sentence). 14 tests. NEXT 35.2: per-workspace rename (agents.display_name + PATCH endpoint + UI).
>
> **Phase 34: Chat-first insights ("ask your business team") — the vision's headline UX.**
> - **34.3 Ask history (depth)** ✅ done (PR #81). The ask box now keeps a thread across refreshes. The questions were already persisted (ask commands) + answers (bi.summary/bi.crash events); `ask.list_recent_asks` stitches them (2 batched queries, newest-first, status answered/failed/pending). `GET /workspaces/me/ask` lists them. `AskBox` loads history on mount + refetches when a new answer resolves, rendering prior Q&A under "Earlier" (de-duped against the live in-flight answer via command_id). 3 new tests; tsc + next build green.
> - **34.2 Proactive feed** ✅ done (PR #80). A unified "team activity" timeline of what the AI team did. `backend/feed.py`: pure `build_feed_item(event)` maps each persona's emitted event (reputation.analyzed/lead.scored/inbox.processed/website.check_complete/marketing.created/bi.summary + *.crash) into an owner-facing item {assistant, title, detail, severity, timestamp}, reusing each persona's own `severity` field ("error" -> alert). `GET /workspaces/me/feed` queries the feed-worthy event kinds (expanding `IN` bindparam) newest-first and maps them. `dashboard/app/feed/page.tsx` renders a day-grouped timeline (red dot = alert) at `/feed`, linked first under the Header "Activity" group; polls every 10s. 13 tests (per-kind mapping incl. alert vs info, drop non-feed, truncation; endpoint ordering + filter + empty). tsc + next build green.
> - **34.1 Ask spine** ✅ done (PR #79). A top-level "Ask your business team" box on the home page routes a plain-English question to the BI assistant's question-mode and renders the answer inline. `backend/ask.py`: `enqueue_question` (drops a `bi.summarize` with `source=ask` + the question + the workspace's recent activity, reusing the new `feeder.gather_recent_activity` so a question answers against the same rollup the digest summarizes) + `get_answer` (polls `bi.summary`/`bi.crash` events by command_id, workspace-scoped). Endpoints `POST /workspaces/me/ask` (+ `bi_assistant_deployed`) and `GET /workspaces/me/ask/{id}` (pending/answered/failed, 404 cross-tenant). `AskBox.tsx` posts then polls (2s, 30 tries), with suggestion chips. 11 tests; tsc + next build green. ASYNC by design (BI runs on the worker); needs the BI assistant deployed + an Anthropic key to actually answer.
>
> **Phase 33: Business-shaped onboarding (the chat-first "answer a few questions -> working team" the vision calls for).** Replaces the developer-shaped README flow for non-technical owners.
> - **33.4 Onboarding UX polish (re-entry + live team status)** ✅ done (PR #78). `/welcome` no longer shows a blank wizard after completion: an already-onboarded workspace gets a summary view ("you're set up for a X business, helping with Y") with an "Adjust my team" path that pre-fills the wizard from the saved profile. New `GET /workspaces/me/team/status` (per-provisioned-persona latest deploy status: running/queued/null + is_llm + needs_anthropic_key) powers a live `TeamStatusPanel` (polls every 4s; green=running, amber=starting) shown on both the result and summary views. 3 endpoint tests; tsc + next build green.
> - **33.3 Industry-tailored persona prompts** ✅ done (PR #77). The LLM personas (bi, marketing, inbox) tailor their system prompt to the owner's industry. Injection: the worker secrets endpoint (`/worker/workspaces/{id}/secrets`) adds `LIGHTSEI_BUSINESS_INDUSTRY` from `onboarding_profile` to the env dict the worker already injects (zero worker change; never overrides a real secret). Each bot gains a pure `_industry_clause` + `_system_prompt(industry)`; `build_prompt(..., industry=None)` reads the env var by default (testable via the kwarg). Re-vendored bi/marketing/inbox into `backend/builtin_personas/` (drift guard green). 8 tests (per-bot tailoring + unknown-industry-no-op + endpoint injects/omits). Verified the env path end-to-end. Heuristic personas (website/lead/reputation) unaffected (no LLM prompt).
> - **33.2.2 Adding the Anthropic key auto-redeploys the AI assistants** ✅ done (PR #93). UX fix (founder: "users won't know how to redeploy"): worker injects secrets at spawn, so a bot started before the key existed couldn't see it. Now `PUT /workspaces/me/secrets/ANTHROPIC_API_KEY` auto-redeploys the running LLM personas (bi/marketing/inbox) from their existing blob (heuristic ones untouched); response carries `redeployed_assistants`. Factored `_redeploy_from` out of the redeploy endpoint. 2 tests.
> - **33.2.1 Fix: auto-deployed bots couldn't authenticate** ✅ done (PR #92). Found during prod verification: onboarded (magic-link) users have no API key, and `team/deploy` didn't create one, so every deployed persona exited on startup ("LIGHTSEI_API_KEY missing — refusing to start", rc=2). Now `deploy_team` mints a workspace API key + stores it as the `LIGHTSEI_API_KEY` secret (idempotent) before deploying, so the bots can authenticate. This was breaking the core onboarding promise for ALL non-technical users. 2 tests.
> - **33.2 Auto-deploy provisioned personas (the "working team" payoff)** ✅ done (PR #75). The backend's build context excludes the repo's top-level `agents/`, so the 6 business personas are vendored into `backend/builtin_personas/<name>/` (byte-identical copies; a drift-guard test enforces they match `agents/`). `builtin_personas.py` builds a deterministic bundle zip (bot.py + requirements.txt at root, fixed timestamps). `POST /workspaces/me/team/deploy` builds + queues a deployment (source='builtin') for each provisioned persona without an active deployment — idempotent, flags `needs_anthropic_key` when an LLM persona is provisioned but no key is set. Wired into `/welcome`: "Build my team" now provisions AND brings the team online, prompting for an Anthropic key if needed. 16 tests (drift guard, deterministic bundle, deploy idempotency, key-flag); tsc + next build green. HONEST LIMIT: queues deployments; the worker brings them to running (and LLM personas need ANTHROPIC_API_KEY to do real work).
> - **33.1 Onboarding spine** ✅ done (PR #74). `backend/onboarding.py`: pure catalog (5 industries + 6 goals, each goal -> persona + feeders + connector) + `build_provisioning_plan` (answers -> assistants/feeders/connectors_needed, dedup + drop-unknown) + `apply_provisioning_plan` (ensure_agent + enable feeders + store profile). `workspaces.onboarding_profile` JSONB (migration 0053). Endpoints `GET/POST /workspaces/me/onboarding`. Dashboard `/welcome` wizard (industry -> pre-checked goals -> "Build my team" -> team + connect-these-next), wired as the home EmptyState primary CTA. 17 tests; tsc + next build green. HONEST LIMIT: provisions assistant *rows* + feeders + profile; the persona bot *code* still deploys via the existing worker bundle flow (the dashboard already handles "provisioned but not deployed" gracefully). NEXT: auto-deploy provisioned personas (needs a built-in-persona bundle source), and industry-tailored persona prompts.
>
> **Phase 32: Business-persona assistants (the AI Business Team roster) — ✅ ROSTER COMPLETE (all 6 shipped 2026-06-07); NOW = feeders + reframe + deploy.** Per the 2026-06-07 product vision (MEMORY.md "Your AI Business Team"), build the customer-facing roster of business-ops assistants that the vision promises — replacing the dev-tooling bots (argus/vega/atlas) as the *product*. Each follows the proven `agents/<name>/bot.py` pattern (pure-function core + `tick()` + mirrored tests), named by business function (clear to a small business owner), starting with the most self-contained + testable. (iOS Phase 31 remains in App Store review in the background — see its sub-plan below.)
>
...
> - **32.4 Inbox assistant** ✅ done (PR #59). `agents/inbox/bot.py` + 11 tests. LLM triager (payload-driven, decision 2026-06-07): claims `inbox.process`, categorizes + flags urgency + drafts a reply + summarizes (structured JSON, tolerant parse), emits `inbox.processed`, pages hermes only on urgent/needs-human. A Gmail-poll feeder that enqueues these is a clean follow-up.
>
> **✅ AI Business Team roster COMPLETE — all 6 personas shipped** (website, lead, reputation, marketing, bi, inbox; 60+ tests). **Deployed + demoed live on prod 2026-06-08:** all 6 deployed to the `lightsei-worker` into the Constellation Demo workspace (set ANTHROPIC_API_KEY for the 3 LLM ones; stopped the leftover dev bots to free worker slots), then each handled a real task end-to-end — website found 4 real broken links on lightsei.com, lead scored a hot lead 80, reputation flagged a negative review, inbox drafted a billing-complaint reply (high/needs-human), marketing wrote real ad copy, BI produced a weekly summary. The vision works. The dev bots (argus/vega/sirius/cassiopeia) stay internal/demo per the vision. Remaining toward the full product: per-persona feeders (Gmail poll for inbox, review-source poll for reputation, a weekly-digest job for bi), the "AI Business Team" dashboard reframe, and deploying the roster to the worker.
> - **32.7 Reframe hook** ✅ done (PR #62). Surfaced the roster as "assistants" with business-friendly display names; kept "constellation" for the team per the founder correction. 18 customer-facing strings across 11 dashboard files.
> - **32.8 Feeder: weekly BI digest** ✅ done (PR #64). The first feeder — makes the team *proactive* (the vision's core promise). `backend/feeder.py`: pure `build_digest_payload(events)` rolls the last 7 days of persona events into a `bi.summarize` `data` payload; `enqueue_digest_for_workspace` / `tick` enqueue an auto-approved digest per workspace running the BI assistant, idempotent within a ~weekly dedup window. Rides the existing scheduler loop (no new service). On-demand `POST /workspaces/me/feeder/digest` (force) + `GET /workspaces/me/feeder/digest/status`. 15 tests; verified end-to-end (feeder rollup → BI bot `build_prompt`). (Also bumped backend CI `timeout-minutes` 10→20: suite grew into the wall at ~9.5min.)
> - **32.9 Feeder dashboard card** ✅ done (PR #65). The feeder's visible face — closes the loop so the owner *sees* the proactive output. `digest/status` enriched to return `latest_summary` (newest `bi.summary` text); `dashboard/app/WeeklyDigestPanel.tsx` on the home page shows the summary + a "Generate now" button + last-run status, quiet-when-empty on a fresh workspace. tsc + next build green.
> - **32.10 Feeder: cost-spike alert** ✅ done (PR #66). Second feeder, proves the pattern generalizes. Internal data only (`runs.cost_usd`), no config, but fires on a *condition not a clock*: pure `detect_cost_spike(this_week, prior_week)` (1.5x ratio, tiny-baseline + zero-prior guards) → on a real spike, `enqueue_cost_alert_for_workspace` asks the BI assistant (question mode) to explain it, with per-assistant breakdown. Distinct `feeder-cost-alert` source dedups independently of the digest; `tick()` runs both feeders. Surfaces through the existing digest card (it's a `bi.summary`). 8 tests.
> - **32.16 Feeder: Website health sweep (first probe-source feeder) + URL capture** ✅ done (2026-06-14, founder ask: "build the website url + feeder slice"). Closes the gap that made Altair say "no business data": the Website assistant (shipped 32.1) was fully built but nothing fed it a URL on a schedule, and onboarding asked "Monitor our website" as a goal without ever collecting the address. New `website_health` feeder (`feeder.py`): `enqueue_website_check_for_workspace` reads the site URL from the feeder config and enqueues one `website.check` per day (time-window dedup, not per-item — it's a recurring sweep of one site, unlike the review/inbox feeders that fan out per new external item). Pure `normalize_website_url` adds a scheme + validates a real host so a bare "example.com" or a typo can't become a target. First **probe** source (no OAuth — the assistant just HTTP-fetches the public site), so it lights up with zero connectors; **default ON** but a clean no-op until a URL is set. Onboarding now captures the URL: the "website" goal carries `needs_url`, `build_provisioning_plan(industry, goals, website_url)` normalizes + carries it only when that goal is selected, `apply_provisioning_plan` stores it as the feeder config. Config endpoint extended to accept `url_target` feeders (URL normalized + 400 on garbage); settings response gains `url_target`. UI: a site-URL field under "Monitor our website" in the `/welcome` wizard + a URL input under the website feeder in `FeedersSettingsSection`. Website checks already roll into Altair's weekly digest (`website.check_complete` → `website_checks` in `_NOTABLE_KINDS`) and the proactive feed. 15 new tests (feeder enqueue/no-url/dedup/window/tick + URL normalize + onboarding plan/apply + endpoint normalize/reject); no migration (reuses `feeder_settings`); backend suite + tsc + next build green.
> - **32.15 Per-workspace feeder targeting** ✅ done (PR #73). Removes the Reputation feeder's "first location only" limit. `feeder_settings.config` JSONB (migration 0052, additive: empty = auto-discover, no backfill); `get_feeder_config`/`set_feeder_config` (a fresh config row inherits the *catalog* default-enabled, so picking a target can't silently turn an off-by-default feeder on). The reputation feeder honors `config.{account_id,location_id}` and skips discovery when set. Endpoints: `PATCH /workspaces/me/feeders/{kind}/config` (400 on non-targetable) + `GET /workspaces/me/feeders/{kind}/targets` (walks the connector accounts→locations as the reputation assistant; returns `available:false` + reason instead of 500 when the connector isn't connected). `FEEDER_CATALOG` entries gain `target_connector`; settings response gains `config` + `targetable`. UI: a location dropdown under targetable feeders in `FeedersSettingsSection` ("auto (first location) until you pick one"). 11 new tests; migration head 0052; tsc + next build green.
> - **32.14 Feeder: Reputation review-poll (Google Business Profile)** ✅ done (PR #72). The last persona feeder — every persona now has a proactive path. `enqueue_reputation_reviews_for_workspace` auto-discovers the target (first account → first location, zero config for the common single-location SMB; the chosen location id rides each command so it's not silent), polls `list_reviews` through the gated connector path *as the reputation assistant*, enqueues one `reputation.check` per new review (deduped permanently on review id). New `reputation_reviews` feeder kind, **default OFF**. 7 new tests (stubbed connector chain); verified end-to-end (feeder payload → reputation bot flags a negative review). No migration. KNOWN LIMIT (documented): multi-location accounts only get their first location until per-workspace targeting lands.
> - **32.13 Google Business Profile connector (review source)** ✅ done (PR #69). Unblocks the Reputation feeder. New `connectors/google_business.py` mirroring the Gmail pattern (MANIFEST + INVOKE + `_request`); reuses the existing Google OAuth flow with the `business.manage` scope (purely additive — registry entry + module, no OAuth-flow change). Tools: `list_accounts`, `list_locations`, `list_reviews` (normalizes star enum → int rating, flags existing replies), `reply_to_review`. Wedge-safe zones (no public). 12 tests (stubbed httpx). Registry-driven, so the OAuth start endpoint + the dashboard integrations grid pick it up automatically. NEXT: the Reputation review-poll feeder that consumes it (needs per-workspace account/location target → the feeder-config-with-targets extension, or auto-discover the first location).
> - **32.12 Feeder: Inbox Gmail-poll (first external-source feeder)** ✅ done (PR #68). Extracted the wedge-critical connector dispatch into `invoke_connector_tool` (one implementation of the capability + trust-zone + install gates; the HTTP endpoint is now a thin wrapper, 15 connector tests confirm behavior preserved). `enqueue_inbox_items_for_workspace` polls Gmail `search_inbox` through that helper *as the inbox assistant* (so the wedge holds), enqueues one `inbox.process` per new message, deduped permanently on Gmail message id. New `inbox_gmail` feeder kind, **default OFF** (`default_enabled` added to the catalog; it polls a real inbox, so it's the owner's call) — the settings toggle appears automatically. 11 new tests (stubbed Gmail). No migration (reuses feeder_settings). Lights up live only when an owner connects Gmail + toggles it on.
> - **32.11 Feeder settings (owner opt-out)** ✅ done (PR #67). Per-workspace on/off for each feeder. `feeder_settings` table (migration 0051, additive: no row = enabled, zero backfill); `FEEDER_CATALOG` + `is_feeder_enabled`/`get_feeder_settings`/`set_feeder_enabled`; `tick()` gates each feeder independently. Endpoints `GET /workspaces/me/feeders` + `PATCH /workspaces/me/feeders/{kind}` (404 on unknown). `FeedersSettingsSection.tsx` toggle list on workspace-settings. 28 feeder tests; single alembic head 0051; tsc + next build green. This is the unlock for config-driven feeders (review source for reputation, Gmail for inbox) — add the `feeder_kind` + a target/schedule column when those land.
>
...
> - 31.2 ✅ Phase 27.5 surface re-audit on prod-built iOS (2026-05-31)
> - 31.3 ✅ TestFlight prerequisites — App Store Connect record + cloud-managed Distribution cert + profile (2026-05-31 → 2026-06-01)
> - 31.4 ✅ TestFlight CI pipeline — GitHub Action greenlights end-to-end on macos-26 (2026-06-01, run #7)
> - 31.5 — App Store metadata + privacy + reviewer creds. All Claude-doable work shipped (drafts in `ios/AppStore/`; export key + account deletion in code); remaining 31.5 items are manual App Store Connect web-UI entry + screenshots, tracked as TODO-operator notes below and gated alongside 31.6.
>   - 31.5.a ✅ draft (2026-06-05) — listing copy: name / subtitle / promo / keywords / description / support+marketing+privacy URLs, within Apple's char limits. `ios/AppStore/listing-copy.md`. TODO operator: paste into App Store Connect; pick subtitle + app-name variant.
>   - 31.5.b ⬅ — Screenshots: 13" iPad (required, app is universal) + 6.9" iPhone. Both sims are booted with the build-9 code installed (team-signed so Keychain works): iPad Pro 13" (930BC65F...) + iPhone 17 Pro Max (A1AD9C66...). Sign in with the demo creds, navigate Agents/Cost/Channels, Cmd+S to capture. Capture iPad from the fixed layout (31.5.h).
>   - 31.5.c ✅ draft (2026-06-05) — App Privacy nutrition labels from the real data model (email / name / chat content / APNS device id; all Linked, none used for Tracking; no analytics or crash SDK). `ios/AppStore/privacy-and-compliance.md`. TODO operator: enter in the App Privacy form (Device ID = declared).
>   - 31.5.d ✅ draft (2026-06-05) — Age Rating answers (all content None, no unrestricted web access, not made for kids -> resolves to 4+). `ios/AppStore/age-rating.md`. TODO operator: fill the questionnaire.
>   - 31.5.e ✅ done (2026-06-07) — Demo account created + seeded on prod: operator account `appreview@lightsei.com` (workspace "Lightsei Demo", id a3a237fe...), signs in via the Business tab "Use password instead" path. Seeded via `/tmp/seed_demo.py` (signup + 3 agents polaris/atlas/hermes + 6 runs w/ ~$0.30 cost spread over recent days + 2 channels), verified visible via API. Credentials are NOT in git (chat + local /tmp only); `ios/AppStore/demo-account.md` holds the process + reviewer-note text. TODO operator: paste creds + reviewer note into App Store Connect App Review Information; verify the login path once on build 8.
>   - 31.5.f ✅ code (2026-06-05) — Export compliance: `ITSAppUsesNonExemptEncryption: false` added to `ios/Lightsei/project.yml` info.properties (PR #34, branch `phase-31.5f-export-compliance`); xcodegen bakes it into the generated Info.plist so the encryption questionnaire stops appearing on upload. App's only crypto is OS HTTPS/TLS + Keychain, so exempt. TODO operator: set Pricing & Availability (free vs paid) in the web UI.
>   - 31.5.g ✅ code (2026-06-06) — In-app account deletion (Apple guideline 5.1.1(v); the app creates accounts, so deletion must be initiatable in-app, "contact support" is not accepted). Backend: `DELETE /auth/account` (operator: deletes owned workspaces + user via cascade) and `DELETE /me/end-user` (end user: cascades sessions/links/push/apns; widget conversations kept with end_user_id nulled). iOS: "Delete account…" in the shared account menu (`SlackShellView`) for both identities with a confirmation dialog; `AuthStore.deleteAccount()` + API methods. Verified: 7 new backend tests pass (56 in the auth/workspace/end-user group, no regression); iOS simulator build SUCCEEDED. TODO operator: none beyond shipping it in the next build.
>   - 31.5.h ✅ code (2026-06-07) — iPad layout polish (PR #38). On regular width the shell panes stretched edge-to-edge over an opaque background, so a short list looked lost in a flat-black void on a 13" screen. Cap pane content to a centered 760pt column (regular size class; iPhone unchanged) + let the starfield show through the channel list (`scrollContentBackground(.hidden)`). `ios/Lightsei/Sources/Views/SlackShellView.swift`. Verified on the iPad Pro 13" sim. Shipped in build 9.
>   - 31.5.i ✅ code (2026-06-07) — Per-agent chat no longer hangs forever (PR #39). A per-agent chat message stays `pending` until a deployed bot instance claims it; with no running instance the row spun on "thinking…" indefinitely (App Review risk + real defect). Bound the poll to ~30s; then show "<agent> isn't responding. It may not be running right now." + Try again. `ios/Lightsei/Sources/Views/OperatorChatView.swift`. Shipped in build 10. (Discovered while screenshotting: also set `ANTHROPIC_API_KEY` on the demo workspace so the LLM-routed **team** channel responds — the per-agent chats need deployed bots, so steer reviewers to the team channel.)
> - 31.6 — Submit + iterate on App Store review:
>   - 31.6.a ✅ (2026-06-07) — Cut tag `ios-v1.0.0` (after bumping `MARKETING_VERSION` 0.1.0 -> 1.0.0, PR #37), build 8. Superseded by build 9 (`ios-v1.0.0-2`, iPad layout) and then **build 10** (`ios-v1.0.0-3`, run #10 `27082368890` green, adds the chat-timeout fix 31.5.i). All Version 1.0.0. **Select build 10** for the submission — it carries export key + spark icon + account deletion + iPad layout + chat-timeout.
>   - 31.6.b ✅ (2026-06-07) — On build 10: demo operator account (`appreview@lightsei.com`) signs in via Business > "Use password instead", workspace populated (3 bots, 6 runs/~$0.30 cost, channels), chat functional (team channel responds; per-agent chats show the graceful not-responding state). Screenshots in `~/Desktop/lightsei-screenshots/` (iPad agents/cost/channels/settings + iPhone agents/cost/channels). All App Store Connect fields entered: metadata, App Privacy labels (Email/Name/User Content/Device ID, Linked, no tracking), Age Rating 4+, Content Rights none, Pricing Free, Privacy Policy URL, demo creds + reviewer note. Final pre-submit checklist verified (URLs 200, demo login 200, key set).
>   - 31.6.c ✅ (2026-06-07) — Submitted for review. Status: Waiting for Review.
>   - 31.6.d ⬅ NOW — Await Apple review (typically ~24-48h). Watch email / Resolution Center. If rejected, triage (common: demo-account access, metadata-matches-app, broken-functionality, privacy) and resubmit. KEEP LIVE during review: api.lightsei.com, app.lightsei.com (/privacy + /support), the demo workspace + its ANTHROPIC_API_KEY with enough credits for the reviewer's chat test. On approval, choose manual or automatic release.
...
- **13.1 Argus — security + secret scanner.** ✅ done (2026-06-07, PR #40). `agents/argus/bot.py` + `backend/tests/test_argus.py` (13 tests). `scan_for_secrets` (AWS/private-key/GitHub/Slack/Stripe/Anthropic/generic-provider tokens + entropy-filtered generic assignments), masks every finding, pages hermes only on high severity.
...
- **15.1 Pure detector module.** ✅ done (2026-06-07, PR #44). `backend/behavioral_rules.py`: `BehavioralViolation` + `detect_loop` / `detect_runaway_tokens` / `detect_escalating_permissions` + `evaluate_behavior`. Pure, no DB. 14 tests.
- **15.2 Storage.** ✅ done (2026-06-07, PR #45). `RunBehavioralViolation` model + alembic 0048 (FK CASCADE, unique (run_id, rule)). 3 schema tests.
...
## Phase 10B: GitHub OAuth + multi-repo ✅ COMPLETE (2026-06-07) — OAuth + multi-repo + per-env branch routing, PRs #48-53
...
Data-model decision (operator, 2026-06-07): **full multi-repo now** — a workspace-level `github_connections` (token) + `github_repos` (N per ws) model, webhook routes by repo. (The webhook already probed by (repo_owner, repo_name); the only blocker was the single-row-per-workspace constraint.)

- **10B.1 OAuth module.** ✅ done (PR #48). `backend/github_oauth.py` (build_authorization_url / exchange_code_for_token / is_configured / new_state; LIGHTSEI_GITHUB_CLIENT_ID/SECRET). 8 tests.
...
- **10B.4 Per-env branch -> agent mapping.** ✅ done (PR #53). `GithubRepoBranchTarget` model + alembic 0050 + branch-target CRUD endpoints + webhook routing: a repo with branch targets deploys only the push branch's mapped agents (main->atlas, staging->atlas-staging); a repo with none keeps the legacy single-tracked-branch behavior. Additive/non-breaking (33 webhook + 5 multirepo tests still green + 5 new branch-target tests).

**Phase 10B COMPLETE** (PRs #48-53): OAuth module + multi-repo schema + OAuth/repo endpoints + webhook cutover + dashboard + per-env branch routing, all functional end-to-end and tested. Operator follow-ups (not code): register a GitHub OAuth App + set LIGHTSEI_GITHUB_CLIENT_ID/SECRET to use OAuth live; watch the first prod push after the cutover (PR #51) deployed.
