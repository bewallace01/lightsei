...
> **(This branch = Phase 39.1, off main. The Phase 37/38 SEO-publish reconciliation lives on the stacked PRs #140/#141/#142 and isn't duplicated here.)**
>
> **Phase 39: CMS publishing (sites NOT hosted from a git repo) — WordPress first.** The git-deploy path (Phase 37, `github_publish`) covers Vercel/Cloudflare/Railway/Netlify. WordPress is the other big family for SMB sites (~40% of the web): no repo, no PR, so we publish via the WordPress REST API. Chosen 2026-06-25 (WordPress over Webflow/Shopify on market share for Lightsei's SMB market).
>   - **39.1 WordPress publish core** ✅ done (2026-06-25). `backend/wordpress_publish.py` (pure, injected `request` seam like `github_publish`): `publish_page_to_wordpress` → `POST {base}/wp-json/wp/v2/pages` authed by an Application Password (HTTP Basic), creates the page as a **draft by default** (the draft in wp-admin is the review gate, mirroring the PR gate on the git path); returns `{id, link, status, edit_url}`. `normalize_base_url` (adds scheme, strips trailing slash + a pasted `/wp-json` root), `page_payload` (title/content required, slug/excerpt optional), clear errors for 401/403 (creds) and 404 (REST API off / not WordPress). 13 pure tests green. NEXT: **39.2** per-workspace WordPress credential storage (encrypted, like the GitHub connection) + a `POST /workspaces/me/wordpress/publish-page` endpoint; **39.3** dashboard (connect a WordPress site + a "Publish to WordPress" path on `/seo`).
>
