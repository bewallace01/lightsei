...
- **15.4 Endpoints + dashboard surface.** Backend ✅ done (PR #46): `GET /runs/{id}/behavior` returns violations + `worst_severity` rollup (none/warn/block). ⬅ NOW — dashboard "behavior" chip (amber warn / red block) on the run + agent views, reading the rollup, with the violation reasons on click. Frontend only.
