...
> - 31.6 ⬅ NOW — Submit + iterate on App Store review:
...
>   - 31.6.b ⬅ NOW — On build 10: demo operator account (`appreview@lightsei.com`) signs in via Business > "Use password instead", workspace populated (3 bots, 6 runs/~$0.30 cost, channels) and chat functional (team channel responds; per-agent chats show the graceful not-responding state). Screenshots captured to `~/Desktop/lightsei-screenshots/` — iPad (agents/cost/channels/settings) + iPhone (agents/cost/channels); team-chat shot still wants a clean retake on a fresh conversation. Remaining operator steps in App Store Connect: select build 10, upload screenshots, enter metadata/privacy labels/age rating/content rights/pricing (drafts in `ios/AppStore/`), Privacy Policy URL `https://app.lightsei.com/privacy`, paste demo creds + reviewer note (steer to the team channel).
>   - 31.6.c — Submit for review.
>   - 31.6.d — Triage rejections (common: privacy-policy URL reachability, demo-account access, metadata-matches-app, IPv6/networking) and resubmit.
