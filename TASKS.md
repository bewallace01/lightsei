...
>   - 31.5.b ⬅ — Screenshots (6.5" or 6.7" iPhone, captured from simulator or device). Operator-only: needs a built app + capture.
...
>   - 31.5.e ✅ draft (2026-06-05) — Demo account MUST be an OPERATOR (Business) account via the "Use password instead" path (end-user sign-in is magic-link / Sign in with Apple only, not reviewer-friendly). `ios/AppStore/demo-account.md`. TODO operator: create the account via web signup, populate the workspace with >= 1 bot + a real conversation (else 2.1 reject), fill the App Review Information fields.
...
>   - 31.6.a ✅ (2026-06-07) — Cut tag `ios-v1.0.0` (after bumping `MARKETING_VERSION` 0.1.0 -> 1.0.0, PR #37). Workflow run #8 (`27077784525`) green: build number 8 exported + uploaded to App Store Connect as Version 1.0.0. Build carries all of this session's changes: export-compliance key, spark app icon, in-app account deletion. TODO operator: once Apple finishes processing, select build 8 for the version.
