...
> **Phase 30.2 — dual identity (operator lane).** 30.1 shipped 2026-05-28 (commit `8d76fc1`): the Slack shell (server rail → channel list → chat pane) behind `ChatDataSource`, wired to the end-user widget chat; verified on the simulator (empty + populated). 30.2 adds the operator identity so the SAME shell serves both: `AuthStore` becomes an identity enum (`.signedOut` / `.endUser` / `.operatorUser`); operator login via `POST /auth/login` (email+password — chosen over magic-link for the MVP: no Resend/deep-link dependency, more reliable; magic-link/Google can layer on later); Keychain gets a second slot so both token types persist; an `OperatorChatSource` lists the operator's workspaces (servers) + agents (bot channels); the chat pane generalizes to a `ChatTarget` so operator channels open a threads-API chat (`/agents/{name}/threads` + `/threads/{id}/messages`) while end-user channels keep the widget chat. Full spec under "Phase 30" below. Next: 30.3 team channel (net-new Polaris-routed backend), 30.4+ dashboard-capability ports.
>
