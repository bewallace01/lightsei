...
> **(This branch carries SEO-publish changes split off the Phase 37/38 thread so they can merge independently of the autopublish PR #140, which owns the fuller Phase 37 reconciliation + 37.10. Stacked: 37.11 + 38.1 (PR #141), then 38.2a on top.)**
>
> **38.2a Framework detection + format defaulting** ✅ done (2026-06-25). The publish UI now defaults the page format to match the repo's framework instead of always HTML. Pure `backend/framework_detect.py`: `detect_framework(page_paths, package_json)` → `next-app` / `next-pages` / `vite-react` / `static` / `unknown` (Next.js by the `next` dep OR the `app/**/page.tsx` App-Router signature; Pages Router only when `next` is present, so Vite+RR's `src/pages` isn't misread), + `default_format_for` (next-* map to the render format; vite-react/static/unknown don't override). Endpoint `GET /workspaces/me/github/repos/{id}/framework` (best-effort: reads package.json + page-file list). Dashboard: `fetchRepoFramework` + a DraftCard effect that, when the publish form opens, sniffs the selected repo and defaults the format (respecting a manual pick via `formatEdited`), with a "Detected Next.js (App Router) — defaulted the format" hint. 9 pure tests (`test_framework_detect.py`, green) + endpoint tests written (`test_framework_endpoint.py`, need Docker). tsc + next build green. **38.2b (deferred until #140 merges):** wire the detected/configured format into auto-publish (37.10) so the background path stops hardcoding `html`.
>
> **37.11 Fix: publish branch-collision is a dead-end error** ✅ done (2026-06-24, found in prod). Re-publishing a page hit the deterministic-branch dedup and showed a red "branch already exists" error with no way forward. Now `github_publish.publish_page_to_repo`, on the branch-create 422, looks up the existing open PR for that branch (`find_open_pr_for_branch`, new GET `/pulls?state=open&head=`) and returns `{pr_url, pr_number, branch, already_open:true}` instead of raising — the `/seo` card shows "this page already has an open pull request" with the link. If the branch exists but has NO open PR (merged / leftover closed-PR branch), a clearer actionable error. `_httpx_request` relaxed to preserve JSON-array bodies (the PR-list endpoint). 2 pure tests added to `test_github_publish.py` (collision→PR, collision-no-PR→error); happy path unaffected; dashboard tsc + next build green.
>
> **38.1 Next.js render targets** ✅ done (2026-06-24). `seo_render`: `next-app` (→ `app/<slug>/page.tsx`, a server component with `export const metadata` for SEO title/description) + `next-pages` (→ `pages/<slug>.tsx`, `next/head` for title/meta). Body HTML embedded via `dangerouslySetInnerHTML` with JSON-escaped string literals (round-trip tested). Dashboard format picker + `PageFormat` union gain the two options. 4 new pure tests in `test_seo_render.py` (all 10 green via scratch harness); dashboard tsc + next build green. Next.js needs NO App.tsx route-wiring (file-based routing), so the existing publish path covers it. NEXT (38.2, on the main thread): framework auto-detection so the format defaults itself + auto-publish uses it.
>
