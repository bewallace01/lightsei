...
> **Phase 30.3 — team channel (Polaris-routed).** 30.2 shipped 2026-05-29 (commit `dbd39d6`): dual-identity AuthStore (`.endUser` / `.operatorUser`), operator email+password login lane, `OperatorChatSource` + `OperatorThreadsAPI` + `OperatorChatView` (threads-API chat with 2s polling for pending assistant rows), `SlackShellView` generalized over `ChatDataSource` + `ChatTarget`, SignInView Customer/Business segmented control. Build green; AuthStore restore did NOT regress the end-user lane (sim test confirmed). 30.3 adds the team channel: a workspace-wide conversation that takes one user message and produces N attributed bot replies. Net-new backend (team_conversations + team_messages schema, dispatch endpoint, orchestrator/router decision on which bots reply). iOS side: pin a "team" channel at the top of each workspace's channel list; new `OperatorTeamChatView` that renders multi-bot attribution. 30.3 sub-task breakdown TBD pending design call (broadcast-to-all vs. Polaris-routed-subset).
>
