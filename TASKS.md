...
- **10B.2 OAuth endpoints.** ⬅ NEXT — `GET /workspaces/me/github/oauth/start` (mints state, returns authorize URL) + `GET /github/oauth/callback` (verify state, exchange code, upsert `github_connections` with the token + github_login, redirect). Plus an "add repo" endpoint (register a `github_repos` row + generate its webhook secret). NEEDS OPERATOR: register a GitHub OAuth App + set LIGHTSEI_GITHUB_CLIENT_ID/SECRET on the backend to test live.
- **10B.3b Webhook cutover.** ⬅ the risky one — point `/webhooks/github` at `github_repos` (+ parent `github_connections` for the token) instead of `github_integrations`. Backfill already makes existing connections' data available, so a careful cutover preserves Bailey's live push-to-deploy. Verify hard before merging.
