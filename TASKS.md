...
- **Worker should retire stale bot instances on redeploy of the same agent**. During the Phase 11.7 demo, `lightsei deploy ./agents/atlas` ran multiple times and each run claimed a fresh worker slot without retiring the previous supervisor. We hit `MAX_CONCURRENT=4` and the wrong (older) atlas bundle kept winning the claim race for `atlas.run_tests` commands — sometimes shipping ❌ Slack messages from a bundle whose failing test had already been fixed. Workaround during the demo was a manual `pkill -f /tmp/lightsei-worker/.../src/bot.py`. Real fix: when a worker claims a deploy whose `agent_name` already has a `running` deployment in the same workspace, mark the previous one's `desired_state=stop` server-side; supervisors poll desired_state and exit cleanly. This becomes more important the moment we ship "deploy from dashboard" (parking-lot item below) — non-terminal users won't know to `pkill`.
