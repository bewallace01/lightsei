...
> **Phase 30.7 — zones surface (fourth iOS dashboard port).** 30.6 (Cost) shipped 2026-05-31 across three sub-commits (`527c33c` Swift API client, `03237e6` single-screen MTD view with budget bar + by-agent + by-model breakdowns, `056e0dd` `Channels | Runs | Agents | Cost` picker). No backend changes needed. 30.7 ports zones: per-zone agent grouping + cross-zone dispatcher callout. Web /zones is a constellation SVG, but it's all computed client-side from /agents. iOS reuses the existing `fetchAgentRows()` API client + `AgentsNavValue.agentDetail` navigation from 30.5; rows in the grouped list push into the existing 30.5.c detail view. No new API client, no backend changes. Picker grows to 5 segments (still fits comfortably on iPhone); planning the picker shape refactor before 30.8 (integrations) since 6+ segments cramp.
>
