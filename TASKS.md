...
>   - 31.6.a ✅ (2026-06-07) — Cut tag `ios-v1.0.0` (after bumping `MARKETING_VERSION` 0.1.0 -> 1.0.0, PR #37). Workflow run #8 (`27077784525`) green: build 8 uploaded as Version 1.0.0 (export-compliance key + spark icon + account deletion). SUPERSEDED by build 9: tag `ios-v1.0.0-2`, run #9 (`27080774666`) green, build 9 adds the iPad layout polish (31.5.h). Both are Version 1.0.0; **select build 9** for the submission.
>   - 31.6.b ⬅ NOW — Verify on that build: demo operator account signs in via Business > "Use password instead", and the workspace shows >= 1 bot + a real conversation. Confirm the listing copy's named surfaces (chat, Activity/Runs view) actually render so metadata matches the app. Also enter the drafted metadata / privacy labels / age rating + capture screenshots (31.5.b) while in App Store Connect.
